({
    doInit: function (component, event, helper) {
        ...
        var action = component.get('c.initFailureData');
        action.setParams({
            recordId: recordId
        });
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                ...
                component.set('v.failureAreaList', result);

                var getObjectAction = component.get('c.getObjectType');
                ...
                helper.showMyToast('Error', '정보를 불러오는 중 에러가 발생하였습니다.');
            }
        })
        $A.enqueueAction(action);
    },

    failureDelete: function (component, event, helper) {
        var rowIndex = event.getSource().get('v.accesskey');
        console.log('rowIndex', rowIndex);
        var failureAreaList = component.get('v.failureAreaList');
        var faId = failureAreaList[rowIndex].Id;
        console.log('faId', faId);
        var action = component.get('c.deletefailureArea');
        action.setParams(
            {
                faId: faId
            }
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == "SUCCESS") {
                ...
                helper.showMyToast('Success', '고장 부위가 삭제되었습니다.')
                $A.getCallback(function() {
                    component.get("c.doInit").run();
                })();    
        } else {
                helper.showMyToast('Error', '고장 부위 삭제가 실패하였습니다.');
            }
        });
        $A.enqueueAction(action);
    },

    registerFailureData: function(component, event, helper) {
        component.set('v.openRegisterFailureData', true);
    },

    handleCancel: function(component, event, helper) {
        component.set('v.openRegisterFailureData', false);
    },
        ...
        action.setCallback(this, function (response) {
            ...
            if (state == 'SUCCESS') {
                ...
                if(result.isSuccess == true) {
                    ...
                    helper.showMyToast('ERROR', '조건이 이미 등록되어 있습니다.')
                    component.set('v.isLoading', false);
                }
            } else {
                // 오류 핸들링
