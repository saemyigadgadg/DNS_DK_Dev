...
({
    ...
    // 2025-02-26 하수헌 추가 Quote, Opportunity를 판단해서 탭닫기
    checkRecord: function(component, recordId) {
        if (recordId.startsWith('0Q0') || recordId.startsWith('006')) {
            console.log('Quote, Opportunity 레코드 ID 감지됨: ', recordId);
            this.navigateQuote(component, recordId);
        } else {
            console.log('Quote, Opportunity ID 아님, 기존 로직 실행');
            this.navigateList(component);
        }
    },
    
    navigateQuote: function(component, quoteId) {
        var workspaceAPI = component.find("workspace");
        workspaceAPI.getFocusedTabInfo().then(function(response) {
            var focusedTabId = response.tabId;
            workspaceAPI.closeTab({tabId: focusedTabId});
