...
({
    ...
    // 부품사용내역 표 추가
    addUsageHistoryList: function (component, event, helper) {
        var usageHistory   = component.get("v.usageHistory");

        var objSelectItem = {
            
        }

        usageHistory.push(Object.assign({}, objSelectItem));
        component.set("v.usageHistory", usageHistory);
    },

    // 부품사용내역 표 삭제
    deleteUsageHistoryList: function (component, event, helper) {
        var usageHistory = component.get("v.usageHistory");
        let updatedUsageHistory = usageHistory.filter((usage) => !usage.usageHistoryChange);

        component.set("v.selectAllUsageHistoryList", false);
        component.set("v.usageHistory", updatedUsageHistory);
    },

      //부품 사용 내역 체크박스 전체 선택
      toggleAllUsageHistory: function(component, event, helper) {
        const selectAll = event.getSource().get("v.checked"); // 체크박스의 상태 가져오기
        let usageHistory = component.get("v.usageHistory");
    
        usageHistory.forEach((usage) => {
            usage.usageHistoryChange = selectAll;
        });
    ...
    doSave: function (component, event, helper) {
        try {
            ...
            .then($A.getCallback(function (result) {
                ...
                .then($A.getCallback(function (result) {
                    ...
                    if (r) {
                        ...
                        console.log('none response data.');
                    }
                }))
                .catch(error => {
            ...
            console.error('Save Error:', JSON.stringify(error, null, 4));
                ...
                helper.apexCall(component, event, helper, 'callOutServiceOrder', { 
                    ...
                    .then($A.getCallback(function (result) {
                        ...
                        if (resParam.O_RETURN.TYPE == 'S') {
                            ...
                            helper.toast('SUCCESS', '오더번호 :  ' + orderNumber + '  작업 내역이 확정되었습니다.');         
