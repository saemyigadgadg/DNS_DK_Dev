...
({
        ...
        if (isChecked) {
            ...
            window.setTimeout(() => {
                ...
                if (value8) {
                    ...
                    $A.util.addClass(inputField1, "error-border");
                }
            }, 0);
        } else {
            ...
            helper.apexCall(component, event, helper, 'upsertServiceReport', { fieldMap: fieldMap })
                .then($A.getCallback(function (result) {
                    ...
                    if (searchService) {
                        ...
                        let requester = (searchService.serviceReportInfo && searchService.serviceReportInfo.requester) || '';
                        ...
                        if ((siteManager || siteManagerPhone) && (requester == siteManager) && (contactPhone == siteManagerPhone)) {
    ...
    isConfirmedTrue: function (component, event, helper) {
        try {
            ...
            if (fileList.length === 0) {
                helper.toast('WARNING','확정 시 파일을 첨부해주세요.');
                return;
            }
            ...
            helper.apexCall(component, event, helper, 'upsertServiceReport', { fieldMap: fieldMap })
                ...
                console.log('confirm response ::: ' , JSON.stringify(r, null ,2));
            ...
            .catch(error => {
                helper.toast('ERROR', '데이터 확정에 실패했습니다.');
                console.error('Save Error:', JSON.stringify(error, null, 2));
                component.set('v.isLoading', false);
            });
        } catch (error) {
    ...
    openServiceManModal : function(component, event, helper) {
        ...
        console.log('test :::: ', JSON.stringify(loginUserInfo, null, 2));
        ...
        $A.createComponent("c:DN_serviceManModal",
            ...
            function (content, status, errorMessage) {
                if (status === "SUCCESS") {
                    ...
                    console.log("서버 또는 클라이언트의 응답이 오프라인입니다.")
                } else if (status === "ERROR") {
                    helper.toast('ERROR', '모달창 활성화 중 오류가 발생했습니다.');
    ...
    clearWorker: function (component, event, helper) {
        ...
        if (!workList || !workList[index].worker) { 
            helper.toast("WARNING", "저장된 작업자가 없습니다.");
            return;
        }
