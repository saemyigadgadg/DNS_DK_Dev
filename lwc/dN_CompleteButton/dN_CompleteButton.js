...
import createResult from '@salesforce/apex/DN_ModelManagerController.createResult'; // CS02일때
import createdResult from '@salesforce/apex/DN_ModelManagerController.createdResult'; // CS02가 아닐때
...
// import getEquipObjType from '@salesforce/apex/DN_ModelManagerController.getEquipObjType';


export default class DN_CompleteButton extends LightningElement {
    ...
    connectedCallback() {
        ...
        getServiceAppointment({ saId: this.recordId})
            .then((result) => {
                if (result === 'IS_DNSA') {
                    console.log('DNSA True12');
                    
                } else if (result === 'IS_COMPLETE_TRUE') {
                    this.showToast('Error', label.DNS_FSL_DispatchCompleted, 'error');
                    this.isModalOpen = false;
                    this.isCompletionModalOpen = true;
                    console.log('flag2');
                } else if (result === 'IS_ORDERTYPE_FALSE') {
                    this.showToast('Success', label.DNS_FSL_OrderTypeNotSupport, 'success');
                    this.handleUpdated();
        ...
        createdResult({ saId: this.recordId})
            .then((result) => {
                if (result === 'IS_PENDING_PROCESS_TRUE') {
                    ...
                    this.isModalOpen = false;
                    this.isCompletionModalOpen = true;
                    this.showToast('Success', label.DNS_FSL_CompleteSuccessful, 'success');
                } else if (result === 'NULL') {
                    console.log('DNSA True');
                } else {
    ...
    handleUpdate() {
        ...
        createResult({ saId: this.recordId, saveObject: JSON.stringify(this.editObject) })
            .then((result) => {
                if (result === 'IS_PENDING_PROCESS_TRUE') {
                    this.showToast('Error', label.DNS_FSL_DispatchPendingProcessed, 'error');
                    this.isModalOpen = false;
